...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        echo "<script>alert('User already exists. Please try with different details my dear user.'); window.location.href = 'user_reg.php';</script>";
    } else {
       ...
       // $password_hashed = password_hash($password, PASSWORD_DEFAULT); // Encrypt the password
        ...
        if ($stmt->execute()) {
            echo "<script>alert('New customer added successfully'); window.location.href = 'user_reg.php';</script>";
        } else {
            echo "<script>alert('Error: " . $stmt->error . "'); window.location.href = 'user_reg.php';</script>";
        }
